...
const AppNav = (props) => {
    ...
    return (
        ...
        <Navbar expanded={expanded} collapseOnSelect={true} expand="sm" bg="dark" variant="dark" fixed="top">
            <Container>
                <Navbar.Brand href="/">Awalmula Catalog</Navbar.Brand>
                <Navbar.Toggle onClick={toggleExpand} aria-controls="responsive-navbar-nav" />
                <Navbar.Collapse id="responsive-navbar-nav">
                    <Nav className="me-auto">
                        <Link href="/"><a role="button" onClick={toggleExpand} className="nav-link">Home</a></Link>
                        <Link href="/products"><a role="button" onClick={toggleExpand} className="nav-link">Products</a></Link>
                        <span onClick={() => { handleShow(); toggleExpand();}} role="button" className="nav-link">Categories</span>
                    </Nav>
                </Navbar.Collapse>
            </Container>
        </Navbar>
    ...
    <ul className={`${s.categoriesUl} ${s.nested} ${!isExpanded && s.active} m-0`}>
        <li><span onClick={toggleOpen} 
            className={`${item.children_data && item.children_data.length > 0 && s.caret} ${isOpen && s.caretDown} mb-2 btn btn-link text-success`}>
                <Link passHref href="/products/category/[category]" as={`/products/category/${item.id}`}><span onClick={closeModal}>{item.name}</span></Link>
            </span>
